...
xml_path = 'D:\\project3\\hat_dataset_yolo3\\data_augmentation\\hat_xml\\'
image_path = 'D:\\project3\\hat_dataset_yolo3\\data_augmentation\\raw_image\\'
# augmented image path,auged xml path == raw xml path
auged_image_path = 'D:\\project3\\hat_dataset_yolo3\\data_augmentation\\auged_image\\'
# class name and output train list txt(each line in the txt includes the image path and box information)
classes = ["hat", "no_hat"]
output_file_dir = 'D:\\project3\\hat_dataset_yolo3\\'

convert_image_path(xml_path, image_path)  # 转换xml文件中的原始图像路径
augmenter = DataAugmentForObjectDetection(max_rotation_angle=30)  # 设定增强类的参数如最大旋转角度
